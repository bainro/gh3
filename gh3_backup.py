...
def notes_worker():
  ...
  while True:
    ...
    for key in notes:
      k.PressKey(key)
    k.PressKey(k.STRUM)
    time.sleep(0.017) # doesn't block other threads!
    k.ReleaseKey(k.STRUM)
    prev_notes = notes
    note_q.task_done()

def infer_worker(interpreter, threshold):
  single_note = np.zeros((80, 80, 3), dtype=np.float32)
  last_strum = time.time()
...
def main():
  ...
  def live_play():
    ...
    threading.Thread(target=infer_worker, args=(interpreter, args.threshold), daemon=True).start()
